...
## 24. Slash Commands“文档支持”与“运行时可用”不一致（2026-02-20）

### 现象
- 用户在飞书里执行 `/clear` 得到 `Unknown skill: clear`。
- 同时 `/compact`、`/context` 可能执行后无文本输出，体验上像“没生效”。

### 根因
1. 官方文档列出的是 Claude Code 的命令能力全集。
2. 在 Agent SDK 里，实际可用命令由每次会话 init 消息中的 `slash_commands` 决定。
3. 当前运行时可用列表中可见 `compact/context/...`，但没有 `clear`，因此透传 `/clear` 会落为普通文本并触发 unknown skill。

### 结论
- 不能仅凭文档判断某个 slash 在当前会话可用。
- 必须以运行时 `slash_commands` 为准，再决定是原生透传还是桥接 fallback。

---

## 25. `/clear` 与 `/compact` 最终策略收敛（2026-02-20）

### 目标
- 既保证命令稳定可用，又尽量复用原生能力。

### 策略
1. `/clear`：桥接层 fallback（稳定优先）
- 不再依赖原生 slash 可用性。
- 通过本地会话代际切换实现真清空：`feishu:{chat_id}:vN -> vN+1`。

2. `/compact`：继续原生透传
- 命令原样送入 SDK，不拼接 memory 上下文。
- 监听 `compact_boundary` 事件；若无文本但检测到边界事件，返回“上下文已压缩完成”。

### 结果
- `/clear` 一定可用且可感知。
- `/compact` 在支持时给出明确回执，降低“执行了但没提示”的困惑。

---

## 26. 新增 `/status commands` 自检命令（2026-02-20）

### 背景
- 之前排障依赖人工翻日志，难以快速判断“当前到底支持哪些 slash command”。

### 方案
- 在飞书桥接层新增 `/status commands`。
- 逻辑：读取最近 `chat-*.jsonl` 的 init 事件，提取并返回 `slash_commands` 列表。
- 若未捕获到 init 事件，返回“未发现可用 slash_commands”。

### 价值
- 用户可在飞书内直接自检当前命令能力。
- 避免反复试错和“文档与运行时不一致”的沟通成本。

---

## 27. CI/CD 部署链路连续故障排查（2026-02-25）

### 现象
在 `Auto Deploy to Server` 工作流中，部署步骤连续出现三类报错：
1. `Host key verification failed.`
2. `git@github.com: Permission denied (publickey).`
3. `dial tcp <server>:22: i/o timeout`

### 根因
1. `known_hosts` 初始化顺序错误  
- 在首次部署（目标目录无 `.git`）分支中，先执行了 `git clone`，后写入 `github.com` host key，导致 host key 校验失败。

2. 服务器重置后 GitHub 访问凭据丢失  
- 服务器已重置，`~/.ssh` 中用于访问 GitHub 的私钥不存在或未匹配，导致 `Permission denied (publickey)`。

3. 网络瞬时抖动  
- `i/o timeout` 发生在连接服务器 22 端口阶段，属于链路层超时，不是仓库权限或脚本逻辑问题。

### 解决方案
1. 调整 workflow 执行顺序（已落地）  
- 文件：`.github/workflows/deploy.yml`  
- 将以下命令前移到首次 `git clone` 之前：
  - `mkdir -p ~/.ssh`
  - `chmod 700 ~/.ssh`
  - `ssh-keyscan -H github.com >> ~/.ssh/known_hosts 2>/dev/null || true`
- 删除 `export GIT_TERMINAL_PROMPT=0` 后面的重复初始化段。
- 对应提交：`634db48`（`fix(cicd): trust github host key before first git clone`）

2. 恢复“服务器 -> GitHub”SSH 权限（操作指引）  
- 在服务器生成/恢复用户级私钥（如 `~/.ssh/id_ed25519`）。  
- 将对应公钥添加到 GitHub 用户 `SSH keys`（或仓库 `Deploy keys`）。  
- 在服务器配置 `~/.ssh/config` 指定 `IdentityFile`，并验证 `ssh -T git@github.com` 可通过。

3. 针对网络抖动的处理  
- 超时场景优先重试 workflow。  
- 若反复出现，再排查 `SERVER_HOST`、安全组、防火墙、`sshd` 服务状态。

### 结果
- 配置问题已拆解为独立层次：
  - Actions -> 服务器（网络/端口）
  - 服务器 -> GitHub（SSH 凭据）
  - 首次 clone 前 host key 信任（脚本顺序）
- 后续排障可按层定位，避免将网络问题与权限问题混淆。

---

## 28. 部署成功但飞书无响应 + 重复部署进程叠加风险（2026-02-25）

### 现象
1. GitHub Actions 显示部署成功，但飞书发消息无响应。  
2. 重复触发部署后，担心出现多个 `uvicorn` / `feishu_ws_bridge.py` 进程并发运行。

### 根因
1. 部署脚本最初仅启动了 `uvicorn`，未启动飞书长连接桥接进程。  
- 由于飞书消息只由 `feishu_ws_bridge.py` 处理，Web API 正常并不代表飞书通道可用。

2. 启动逻辑为“直接 nohup 后台启动”，缺少幂等重启控制。  
- 再次部署时可能保留旧进程并额外拉起新进程，造成：
  - 端口占用冲突（`uvicorn`）
  - 多实例重复消费/重复回复（`feishu_ws_bridge.py`）
...
- 对应提交：`81990b3`（`ci: start feishu websocket bridge during deploy with pid/log`）

2. 将部署改为“先停后起 + 存活校验”的安全幂等流程（已落地）  
- 文件：`.github/workflows/deploy.yml`
- 关键策略：
  - 先按 pid 文件停止旧 `uvicorn` / `feishu_ws_bridge`（若存在）
  - 再用 `pgrep -f` 兜底清理残留进程
  - 启动新进程后 `sleep + kill -0` 检查存活
  - 启动失败时输出对应日志并 `exit 1`
- 对应提交：`57de49f`（`ci: make deploy idempotent by restarting and verifying services`）

### 结果
- 飞书桥接会随部署自动启动，`.env` 位于项目根目录即可被桥接读取。  
- 重复触发部署不会累计多实例，部署行为具备幂等性与可观测性。  
- 若服务拉起失败，CI 会明确失败并回传日志片段，便于快速定位。
